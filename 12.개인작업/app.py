...
@app.route('/')
def home():
    menu = {'home':1, 'menu':0, 'map':0, 'excel':0}
    return render_template('index.html', menu=menu)

@app.route('/menu', methods=['GET','POST'])
def menu():
    menu = {'home':0, 'menu':1, 'map':0, 'excel':0}
...
@app.route('/map')
def map():
    menu = {'home':0, 'menu':0, 'map':1, 'excel':0}
    filename = os.path.join(app.static_folder, 'img/광주시_자동차정비업체.html')
    mtime = int(os.stat(filename).st_mtime)
    return render_template('map.html', menu=menu, mtime=mtime)

@app.route('/excel')
def excel():
    menu = {'home':0, 'menu':0, 'map':0, 'excel':1}
    return render_template('excel.html', menu=menu)
